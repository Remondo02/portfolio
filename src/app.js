function init() {
  ...
  const smoothScroll = () => {
    const scroll = new SmoothScroll(
      '.navbar, .about__details__contact, a[href*="#"]',
      {
        speed: 800
      }
    );
  ...
  const lightBoxContainer = () => {
    ...
    if (isIE) {
      ...
      const images = document.querySelectorAll(
        ".portfolio__gallery__container img"
      );
      /**
      ...
      images.forEach(image => {
        const titles =
          image.parentElement.parentElement.childNodes[3].firstElementChild;
        const content =
          image.parentElement.parentElement.childNodes[3].childNodes[3];
        const link =
          image.parentElement.parentElement.childNodes[3].childNodes[5];
        const modalClose =
          image.parentElement.parentElement.childNodes[3].childNodes[6]
            .nextElementSibling;
        /**
